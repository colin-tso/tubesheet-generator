...
export const TUBE_SHEET_LAYOUTS = [30, 45, 60, 90, "radial"];
...
const TUBE_SHEET_LAYOUT_SET = new Set(TUBE_SHEET_LAYOUTS);
export class TubeSheet {
    ...
    computeGeneratedProps() {
        if (!TUBE_SHEET_LAYOUT_SET.has(this._layout)) {
            throw new Error(`Invalid tube layout: ${String(this._layout)}`);
        }
        let minID = null;
        let effectiveShellID = null;
        if (this._shellID) {
            effectiveShellID = this._shellID;
        }
        else if (this._minTubes) {
            minID = findMinID(this._minTubes, this._OTLClearance, this._tubeOD, this._pitchRatio, this._layout);
            effectiveShellID = minID;
        }
        if (effectiveShellID === null) {
            return { minID, numTubes: 0, tubeField: null, OTL: null };
        }
        const tubeField = generateTubeField(effectiveShellID, this._OTLClearance, this._tubeOD, this._pitchRatio, this._layout);
        const numTubes = tubeField ? tubeField.length : 0;
        if (this._shellID) {
            minID = findMinID(numTubes, this._OTLClearance, this._tubeOD, this._pitchRatio, this._layout);
        }
 ...
 *                             `magnitude`.
 */
const ulpTolerance = (magnitude, ulps = 64) => {
 ...
 */
const isTube = (value) => typeof value === "object" && value !== null && "x" in value && "y" in value;
/**
 ...
 */
const memoizeBounded = (fn, resolver, cacheSize) => {
    const memoized = memoize(fn, resolver);
    memoized.cache = new LRUCache(cacheSize);
    return memoized;
};
 ...
 */
const expandQuarterField = (quarterTubeField) => {
    const xAxisMags = new Set(); // stores |x| for points on y===0
    const yAxisMags = new Set(); // stores |y| for points on x===0
    const core = [];
    let hasOrigin = false;
 ...
 */
const generateTubeField = memoizeBounded((shellID, OTLClearance, tubeOD, pitchRatio, layout, offsetOption = "AUTO") => {
    try {
        if (shellID <= 0) {
        ...
        const maxCentreDistSq = (maxCentreDist + BOUND_TOLERANCE) * (maxCentreDist + BOUND_TOLERANCE);
...
const scanQuarterField = memoizeBounded((shellID, OTLClearance, tubeOD, pitchRatio, layout, offsetOption) => {
    try {
        ...
        const maxCentreDistSq = (maxCentreDist + BOUND_TOLERANCE) * (maxCentreDist + BOUND_TOLERANCE);
        let hasOrigin = false;
        const xAxisMags = new Set();
        const yAxisMags = new Set();
        let coreCount = 0;
        let maxDistSq = 0;
 ...
 */
const RADIAL_SEED_COUNTS = [1, 2, 3, 4, 5];
...
const ringTubeCount = (radius, pitch) => {
    ...
    while (2 * radius * Math.sin(Math.PI / numTubes) <
        pitch - ulpTolerance(Math.max(radius, pitch))) {
        numTubes--;
    }
    return numTubes;
};
const placeRing = (radius, tubeField, pitch) => {
...
const buildSeedField = (count, pitch, maxCentreDist) => {
 ...
 */
const seedTubeCount = (count, pitch, maxCentreDist) => {
    const seedRadius = count === 1 ? 0 : pitch / (2 * Math.sin(Math.PI / count));
    if (seedRadius > maxCentreDist + ulpTolerance(Math.max(seedRadius, maxCentreDist))) {
        return 0;
    }
 ...
 */
const radialTubeField = (shellID, OTLClearance, tubeOD, pitchRatio) => {
    const pitch = tubeOD * pitchRatio;
    const maxOTL = shellID - OTLClearance;
    const maxCentreDist = (maxOTL - tubeOD) / 2;
 ...
 *                                                  tube sheet.
 */
const tubeCount = (shellID, OTLClearance, tubeOD, pitchRatio, layout, offsetOption = "AUTO", lazy = false) => {
    if (lazy) {
        if (layout === "radial") {
            const pitch = tubeOD * pitchRatio;
            const maxOTL = shellID - OTLClearance;
        ...
        if (offsetOption === "AUTO") {
            const resultTrue = scanQuarterField(shellID, OTLClearance, tubeOD, pitchRatio, layout, true);
        ...
        const result = scanQuarterField(shellID, OTLClearance, tubeOD, pitchRatio, layout, offsetOption);
 ...
 */
const OTLFromTubeField = (tubeField, tubeOD) => {
 ...
 *                                                  invalid.
 */
const tubeFieldOTL = (shellID, OTLClearance, tubeOD, pitchRatio, layout, offsetOption = "AUTO", lazy = false) => {
    try {
...
export const findMinID = memoizeBounded((minTubes, OTLClearance, tubeOD, pitchRatio, layout, offsetOption = "AUTO") => {
    ...
    const BOUND_MAX_ITERATIONS = 1000;
    if (tubeOD <= 0) {
    ...
    if (layout === "radial") {
        ...
        while (tubeCount(upperBound, OTLClearance, tubeOD, pitchRatio, layout, "AUTO", true) <
            ...
            if (++boundIterations > RADIAL_MAX_BOUND_ITERATIONS) {
                throw new Error("findMinID: unable to bound a valid diameter within the iteration limit.");
            }
        }
        ...
        return roundUp(OTL !== null && OTL !== undefined ? OTL + OTLClearance : upperBound, DECIMAL_PLACES);
    }
    while (true) {
        try {
            iterations = 0;
            if (offsetOption === "AUTO") {
                ...
                const updateBounds = (D, numTubes, targetTubes) => {
                    if (numTubes < targetTubes) {
                        ...
                        const OTL = tubeFieldOTL(D, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                ...
                while (tubeFieldOTL(D_old, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) === null &&
                    growOldIterations < BOUND_MAX_ITERATIONS) {
                    D_old = D_old * BETA;
                    if (!Number.isFinite(D_old) || D_old <= 0) {
                        throw new Error("findMinID: diameter guess became non-finite while searching for a valid tubefield.");
                    }
                    growOldIterations++;
                }
                if (growOldIterations >= BOUND_MAX_ITERATIONS) {
                    throw new Error("findMinID: unable to find a valid diameter within the iteration limit.");
                }
                // Save first guess of tube count into memory
                D_old =
                    tubeFieldOTL(D_old, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance;
                numTubes_old = tubeCount(D_old, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                // Increment diameter, save second guess of tube count into
                // memory
                D_new = D_old * BETA;
                D_new =
                    tubeFieldOTL(D_new, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance;
                numTubes_new = tubeCount(D_new, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                ...
                while (numTubes_new !== minTubes && iterations < HEURISTIC_MAX_ITERATIONS) {
                    ...
                    if (iterations > 1) {
                        ...
                        if (numTubes_new > minTubes) {
                            D_check = roundUp(tubeFieldOTL(D_new, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance, DECIMAL_PLACES);
                            numTubes_check = tubeCount(D_check - Math.pow(10, -DECIMAL_PLACES), OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                ...
                if (numTubes_new === minTubes) {
                    return roundUp(tubeFieldOTL(D_new, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance, DECIMAL_PLACES);
                }
                ...
                if (!haveLowerBound) {
                    ...
                    while (shrinkIterations < BOUND_MAX_ITERATIONS) {
                        if (!Number.isFinite(D_shrink) || D_shrink <= 0) {
                            throw new Error("findMinID: diameter guess became non-finite while searching for a lower bound.");
                        }
                        ...
                        const numTubesLower = tubeCount(D_shrink, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                    ...
                    if (!haveLowerBound) {
                        throw new Error("findMinID: unable to find a valid lower bound within the iteration limit.");
                    }
                }
                if (!haveUpperBound) {
                    ...
                    while (growIterations < BOUND_MAX_ITERATIONS) {
                        ...
                        D_grow = D_grow * BETA;
                        const numTubesUpper = tubeCount(D_grow, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                        if (numTubesUpper > minTubes) {
                            D_upperBound = roundUp(tubeFieldOTL(D_grow, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance, DECIMAL_PLACES);
                            haveUpperBound = true;
                            break;
                        }
                        growIterations++;
                ...
                while (D_upperBound - D_lowerBound > Math.pow(10, -DECIMAL_PLACES) &&
                    ...
                    const numTubesMid = tubeCount(D_mid, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true);
                    if (numTubesMid >= minTubes) {
                        D_upperBound = roundUp(tubeFieldOTL(D_mid, OTLClearance, tubeOD, pitchRatio, layout, offsetOption, true) + OTLClearance, DECIMAL_PLACES);
                    }
                    else {
                        D_lowerBound = D_mid;
                    }
 ...
 */
export const getEffectiveShellID = (ts) => {
 ...
 */
const parseSVGStyleString = (svgStyles) => {
    return svgStyles.split(";").reduce((acc, style) => {
...
export const generateTubeSheetSVG = (ts, options) => {
    ...
    const generateSVGCenteredCross = (diameter, svgStyles) => {
        ...
        // Create variables to define bounding box based on coordinates and
        // diameter
        const minX = (-diameter / 2) * 1.1, minY = (-diameter / 2) * 1.1, maxX = (diameter / 2) * 1.1, maxY = (diameter / 2) * 1.1;
